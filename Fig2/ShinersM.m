function M = ShinersM(N)

%Function to simulate the Migration model. On each time step of the
...
at=0.0005; %attraction to migration target.
g=0.0025; %attraction to global center of mass
ba=3.14; %blind angle
...
for i=1:N 
    ...
    P(i,2)=60*(0.5-rand); % Initiates y-coordinate.
...
for k=1:t %Simulate for t time steps
        ...
        for i=1:N % Go through every particle.
            ...
            MI=[1/sqrt(2),1/sqrt(2)]; %Direction to migration target
            
            GA=[mean(P(:,1))-P(i,1),mean(P(:,2))-P(i,2)]; %Direction to GCM

            Dir=d*D+c*C+rr*F+at*MI+g*GA+e*E; %Heading update formula
